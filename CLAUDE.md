...
Server component that fetches the user's profile + all trips (with nested
days/activities for scoring). Sections:

- **Greeting** — first name from `user_profiles.display_name` (falls back to
  the email local-part).
- **UsageBanner** (free users only) — trips used out of the 3-trip free cap.
- **UpcomingTripCard** — the soonest trip with a future start date (fallback:
  nearest start, then most-recent). Shows a days-to-go countdown, a
  `ScoreRing` completion %, and a **WeatherWidget** (Open-Meteo, no key:
  geocode → forecast in `lib/weather/openMeteo.ts`, cached, fails to null).
- **My trips** grid — `TripCard` per trip with status badge (Planning /
  Upcoming / Completed, derived from `status` + dates), completion bar, and
  Open / Duplicate / Delete actions (`duplicateTrip`, `deleteTrip` in
  `lib/trips/actions.ts`).
- **NewTripButton** — modal destination picker → `createTrip(slug)`.

Trip completion score lives in `lib/trips/score.ts` (`completionScore`): 20%
basics + 40% day coverage vs. date range + 40% days-with-activities.

## Account settings (`/account`)

`AccountSettings` (client) drives server actions in `lib/account/actions.ts`:
`updateDisplayName`, `updateEmail` (Supabase confirm email), `updatePassword`
(re-verifies current password), `updateCurrency`, and `deleteAccount` (requires
typing `DELETE`; hard-deletes via the service-role admin client, FK-cascades
all user data, then signs out). Currency options live in `lib/currency.ts` and
match the budget FX list; the saved currency is the default in the budget
expense panel.

## Referral programme

Schema in `supabase/migrations/0007_account_referrals.sql`: adds
`display_name`, `currency`, `referral_code` (unique, auto-generated by
`gen_referral_code()` in `handle_new_user` + backfilled) to `user_profiles`,
and a `referrals` table (`referrer_id`, `referred_user_id` unique, `status`
pending→completed→rewarded). SECURITY DEFINER RPCs: `record_referral(code)`
(rejects self/double referral) and `complete_referral()` (pending→completed,
returns referrer id).

- **Capture**: `?ref=CODE` is stashed in a `wl_ref` cookie by middleware (so it
  survives the OAuth round-trip) and passed as a hidden field on the signup
  form. Recorded at sign-up (`signUpWithEmail` + `auth/callback`) → pending row.
- **Reward**: on onboarding completion, `completeReferralAndReward()`
  (`lib/referrals/actions.ts`) flips the row to completed and calls
  `grantReferralReward()` (`lib/stripe/referral.ts`): ensures a 100%-off
  `once` coupon and applies it to the referrer's subscription (or customer for
  their next invoice), then marks the row `rewarded`. Degrades to no-op without
  Stripe.
- **`/refer`** — referral link + share buttons (copy / WhatsApp / email) +
  a status table of people referred.

## Polish & platform

- **Loading skeletons**: `loading.tsx` for every data-fetching route
  (dashboard, account, account/billing, refer, trip/[id], trip/[id]/budget,
  pricing, invite/[token]) built on `components/ui/Skeleton`.
- **Error boundaries**: per-segment `error.tsx` (dashboard, account, refer,
  trip) + root `app/error.tsx`, `app/global-error.tsx`, and a friendly
  `not-found.tsx`, all sharing `components/ui/ErrorState`.
- **Cookie consent (GDPR)**: `CookieConsent` banner gates analytics via
  `lib/analytics.ts` — nothing is tracked until the user accepts; `Analytics`
  fires consent-gated pageviews. Both mounted in the root layout, which also
  has a skip-to-content link. `/privacy` documents the policy.
